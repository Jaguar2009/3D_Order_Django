...
class EditPostForm(forms.ModelForm):
    ...
    def clean_media_files(self):
        media_files = self.files.getlist('media_files')
        remaining_media_files = self.data.getlist('existing_media_files')

        total_files = len(media_files) + len(remaining_media_files)
        if total_files == 0:
            raise ValidationError("Медіафайли є обов'язковими.")
        if total_files > 10:
            raise ValidationError("Максимальна кількість медіафайлів - 10.")
    ...
    def clean_object_3d_files(self):
        object_3d_files = self.files.getlist('object_3d_files')
        remaining_object_files = self.data.getlist('existing_object_files')

        total_files = len(object_3d_files) + len(remaining_object_files)
        if total_files == 0:
            raise ValidationError("Файли 3D об'єктів є обов'язковими.")
        if total_files > 50:
            raise ValidationError("Максимум 50 файлів 3D об'єктів.")
    ...
    def __init__(self, *args, existing_media_files=None, existing_object_files=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.existing_media_files = existing_media_files or []
        self.existing_object_files = existing_object_files or []
